...
import { FileText, Image as ImageIcon, FileSpreadsheet, File, MessageSquare, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function AdminTicketList({ initialTickets, currentAgentId }: Props) {
    ...
    return (
        <div className="bg-white shadow-md rounded-xl overflow-hidden border border-slate-200">
            ...
            <div className="p-5 border-b border-gray-200 flex flex-col justify-between items-start lg:flex-row lg:items-center bg-white gap-4">
                ...
                <div className="flex bg-gray-100 p-1 rounded-lg">
                    <button
                        ...
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'TODOS' ? 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`}
                    >
                        Todos
                    </button>
                    <button
                        onClick={() => setFilter('PENDIENTES')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'PENDIENTES' ? 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`}
                    >
                        Pendientes
                    </button>
                    <button
                        onClick={() => setFilter('COLA')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'COLA' ? 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`}
                    >
                        Cola de Trabajo
                    </button>
                    <button
                        onClick={() => setFilter('MIS_TICKETS')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'MIS_TICKETS' ? 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`}
                    >
                        Mis Tickets
                    </button>
                    <button
                        onClick={() => setFilter('RESUELTOS')}
                        className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${filter === 'RESUELTOS' ? 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-200'}`}
                    >
                        Resueltos
                    </button>
                </div>
                                            ...
                                            {ticket.vencimiento_sla && (() => {
                                                ...
                                                if (isResolved) {
                                                    ...
                                                    return resolvedAt <= sla
                                                        ? <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700 tracking-wide uppercase">SLA Cumplido</span>
                                                        : <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 tracking-wide uppercase">SLA Incumplido</span>;
                                                }
                                                const diffHours = (sla.getTime() - new Date().getTime()) / (1000 * 60 * 60);
                                                if (diffHours < 0) return <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700 tracking-wide uppercase">Vencido hace {Math.abs(Math.round(diffHours))}h</span>;
                                                if (diffHours <= 12) return <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700 tracking-wide uppercase">Vence en {Math.round(diffHours)}h</span>;
                                                return <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700 tracking-wide uppercase">Vence en {Math.round(diffHours)}h</span>;
                                            })()}
                                        </div>
                                    </td>
